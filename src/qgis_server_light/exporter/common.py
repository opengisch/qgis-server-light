...
def create_full_pg_service_conf(pg_service_conf: str | None = None) -> dict:
    ...
    try:
        logging.info("Loading pg_service configs from ENVIRONMENT variable PGSERVICEFILE:")
        ...
        logging.info("  No service config loaded")
        logging.debug(f"  {e}")
    full_pg_service_config_passed = {}
    if pg_service_conf:
        try:
            logging.info("Loading pg_service configs from passed path")
            full_config = pgserviceparser.full_config(conf_file_path=Path(pg_service_conf))
            for section in full_config.sections():
